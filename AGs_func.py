...
class Poblacion:
    ...
    def cruzar_individuos(self, parental_1, parental_2, verbose=False):
        ...
        descendencia.valor_variables = np.repeat(
            None, descendencia.n_variables)
    ...
    def seleccionar_individuo(self, n, return_indices=True,
        ...
        if metodo_seleccion == "ruleta":
            ...
            probabilidad_seleccion = probabilidad_seleccion / \
                np.sum(probabilidad_seleccion)
            ind_seleccionado = np.random.choice(
                a=np.arange(self.n_individuos),
                size=n,
                p=list(probabilidad_seleccion),
        ...
        if(return_indices):
            ...
            if n > 1:
                return(
                    [copy.deepcopy(self.individuos[i])
                     for i in ind_seleccionado]
                )
    ...
    def crear_nueva_generecion(self, metodo_seleccion="tournament",
        ...
        if elitismo > 0:
            ...
            elite = [copy.deepcopy(self.individuos[i])
                     for i in rank[:n_elitismo]]
    ...
    def optimizar(self, funcion_objetivo, optimizacion, n_generaciones=50,
        ...
        for i in np.arange(n_generaciones):
            ...
            self.historico_mejor_fitness.append(
                copy.deepcopy(self.mejor_fitness))
        ...
        print("Valor óptimo de las variables: " +
              str(self.valor_variables_optimo))
        print("Valor función objetivo: " + str(self.valor_funcion_optimo))
        print("")
        np.savetxt('values_P.txt',
                   self.historico_mejor_valor_variables, delimiter='    ')
        return self.historico_mejor_valor_variables
...
### funcion objetivo del controlador Proporcional
def funcion_objetivo_C_P(kp):
    num = 0.006679
...
def funcion_objetivo_C_PI(x_0, x_1):
    num = 0.90888
    den = [52.32874, 1]
    numc = [x_0 * x_1, x_1]
    denc = [x_1, 0]
...
def funcion_objetivo_C_PID(kp, ki, kd):
    num = 0.006679
    den = [1, 0.2547, 0.008125]
    G = co.tf(num, den)
    ''' Funcion de transferencia realimentada con controlador '''
